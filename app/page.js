...
function pickQs(bank) {
  ...
  ["logic","numerical","spatial","verbal","memory","abstract"].forEach(d => {
    ...
    const e = pool.filter(q => q.difficulty < 0.5), m = pool.filter(q => q.difficulty >= 0.5 && q.difficulty < 0.75), h = pool.filter(q => q.difficulty >= 0.75);
    const p = []; if (e[0]) p.push(e[0]); if (m[0]) p.push(m[0]); if (h[0]) p.push(h[0]);
    const rem = pool.filter(q => !p.includes(q)); while (p.length < QPD && rem.length) p.push(rem.shift());
    sel.push(...p);
  });
...
function calcIQ(res, qs) {
  ...
  let wC=0,tW=0,sB=0;
  res.forEach(r => { const q=qs.find(x=>x.id===r.id); if(!q) return; const w=.5+q.difficulty*2; tW+=w; if(r.correct){wC+=w; const t=r.time/q.timeLimit; if(t<.25)sB+=.18*w; else if(t<.4)sB+=.1*w; else if(t<.6)sB+=.04*w;}});
  const raw=(wC+sB)/tW, est=65+raw*80, se=15/Math.sqrt(res.length)*(1.1+(1-raw)*.4), z=1.282;
  const ds={};
  ["logic","numerical","spatial","verbal","memory","abstract"].forEach(d=>{const dr=res.filter(r=>{const q=qs.find(x=>x.id===r.id);return q&&q.domain===d;});if(dr.length){let dw=0,dt=0;dr.forEach(r=>{const q=qs.find(x=>x.id===r.id);const w=.5+q.difficulty*2;dt+=w;if(r.correct)dw+=w;});ds[d]=Math.round(dw/dt*100);}});
  ...
  return{estimate:Math.round(est),lower:Math.round(est-z*se),upper:Math.round(est+z*se),rawScore:Math.round(raw*100),domainScores:ds,totalCorrect:res.filter(r=>r.correct).length,totalQuestions:res.length,avgTime:Math.round(res.reduce((a,r)=>a+r.time,0)/res.length),strengths:str,weaknesses:wk};
}

const iqLabel=(iq,t)=>{const l=t.iqLabels;if(iq>=145)return l.exceptional;if(iq>=130)return l.superior;if(iq>=115)return l.aboveAvg;if(iq>=85)return l.average;if(iq>=70)return l.belowAvg;return l.sigLow;};
const pctl=(iq)=>Math.min(99,Math.max(1,Math.round(50*(1+Math.sign((iq-100)/15)*Math.sqrt(1-Math.exp(-2*((iq-100)/15)**2/Math.PI))))));
...
export default function NexusIQ(){
  ...
  const timerR=useRef(null), nextR=useRef(null);
  ...
  useEffect(()=>{
    if(phase==="test"&&q){
      setTimeLeft(q.timeLimit);setStartTime(Date.now());setSelected(null);setShowFB(false);setFadeIn(false);
      requestAnimationFrame(()=>setTimeout(()=>setFadeIn(true),30));
      timerR.current=setInterval(()=>setTimeLeft(p=>{if(p<=1){clearInterval(timerR.current);return 0;}return p-1;}),1000);
      return()=>{clearInterval(timerR.current);clearTimeout(nextR.current);};
    }
  },[phase,currentQ]);
  ...
  const doSelect=(idx)=>{
    if(selected!==null||showFB)return;clearInterval(timerR.current);setSelected(idx);
    const el=Math.min((Date.now()-startTime)/1000,q.timeLimit);
    const nr={id:q.id,correct:idx===q.correct,time:el,timedOut:false};
    setResults(p=>[...p,nr]);setShowFB(true);nextR.current=setTimeout(()=>goNext([...results,nr]),1100);
  };
  ...
  const css=`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@300;400;500;600;700&family=Outfit:wght@200;300;400;500;600&family=IBM+Plex+Mono:wght@300;400;500&display=swap');*{box-sizing:border-box;margin:0;padding:0}button{outline:none;border:none;cursor:pointer;font-family:inherit}input{outline:none;border:none;font-family:inherit}@keyframes fadeUp{from{opacity:0;transform:translateY(24px)}to{opacity:1;transform:translateY(0)}}@keyframes fadeIn{from{opacity:0}to{opacity:1}}@keyframes spin{to{transform:rotate(360deg)}}@keyframes pulse{0%,100%{opacity:.4}50%{opacity:1}}@keyframes slideWidth{from{width:0%}}@keyframes countUp{from{opacity:0;transform:scale(.8)}to{opacity:1;transform:scale(1)}}::selection{background:rgba(212,168,67,.3)}`;
  const bg={minHeight:"100vh",background:"#08080C",color:"#E2E0DB",fontFamily:"'Outfit',sans-serif",position:"relative",overflow:"hidden"};
  const gr={position:"fixed",top:0,left:0,right:0,bottom:0,pointerEvents:"none",zIndex:0,background:"radial-gradient(ellipse at 15% 50%,rgba(212,168,67,.04) 0%,transparent 60%),radial-gradient(ellipse at 85% 30%,rgba(69,183,170,.03) 0%,transparent 50%)"};
  const ct={position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",padding:"48px 24px"};

  // LANG SELECT
  if(phase==="lang"){return(
    <div style={bg}><style>{css}</style><div style={gr}/>
      <div style={{...ct,animation:"fadeUp .8s ease-out"}}>
        <div style={{fontSize:14,fontWeight:500,letterSpacing:"4px",color:"#D4A843",marginBottom:48}}>NEXUS<span style={{color:"#45B7AA"}}>IQ</span></div>
        <h1 style={{fontSize:"clamp(24px,5vw,36px)",fontFamily:"'Playfair Display',serif",fontWeight:300,textAlign:"center",marginBottom:12}}>{i18n.es.selectLang}</h1>
        <p style={{fontSize:14,color:"#4A4840",marginBottom:48}}>Select your language</p>
        <div style={{display:"flex",gap:20,flexWrap:"wrap",justifyContent:"center"}}>
          {[{c:"es",f:"🇪🇸",l:"Español",s:"Spanish"},{c:"en",f:"🇬🇧",l:"English",s:"Inglés"}].map(x=>(
            <button key={x.c} onClick={()=>{setLang(x.c);setPhase("landing")}} style={{width:180,padding:"32px 24px",background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.06)",borderRadius:14,color:"#E2E0DB",display:"flex",flexDirection:"column",alignItems:"center",gap:12,transition:"all .3s"}}
              onMouseEnter={e=>{e.currentTarget.style.background="rgba(255,255,255,.05)";e.currentTarget.style.borderColor="rgba(212,168,67,.3)";e.currentTarget.style.transform="translateY(-4px)";}}
  ...
  if(phase==="landing"){return(
    <div style={bg}><style>{css}</style><div style={gr}/>
      <div style={{...ct,animation:"fadeUp .9s ease-out"}}>
        <div style={{position:"absolute",top:28,left:32,fontSize:13,fontWeight:500,letterSpacing:"4px",color:"#D4A843"}}>{t.brand}<span style={{color:"#45B7AA"}}>{t.brandAccent}</span></div>
        <button onClick={()=>setPhase("lang")} style={{position:"absolute",top:28,right:32,fontSize:12,color:"#4A4840",background:"rgba(255,255,255,.03)",padding:"6px 14px",borderRadius:4,border:"1px solid rgba(255,255,255,.06)"}}>{lang==="es"?"🇪🇸 ES":"🇬🇧 EN"}</button>
        <div style={{fontSize:11,letterSpacing:"8px",textTransform:"uppercase",color:"#45B7AA",fontWeight:400,marginBottom:28,opacity:.8}}>{t.tagline}</div>
        <h1 style={{fontSize:"clamp(38px,7vw,64px)",fontFamily:"'Playfair Display',serif",fontWeight:300,textAlign:"center",lineHeight:1.12,letterSpacing:"-1.5px",margin:"0 0 20px",maxWidth:700}}>
          {t.heroTitle1}<br/><span style={{fontWeight:600,background:"linear-gradient(135deg,#D4A843,#E8C96A)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{t.heroTitle2}</span>
        </h1>
        <p style={{maxWidth:480,textAlign:"center",fontSize:15,lineHeight:1.75,color:"#7A786F",fontWeight:300,margin:"0 0 44px"}}>{t.heroDesc}</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,maxWidth:440,width:"100%",marginBottom:44}}>
          {Object.keys(D_COLORS).map((k,i)=>(<div key={k} style={{padding:"14px 12px",borderRadius:10,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",textAlign:"center",animation:`fadeUp .6s ease-out ${.1+i*.08}s both`}}><div style={{fontSize:16,marginBottom:6,opacity:.7}}>{D_ICONS[k]}</div><div style={{fontSize:"10.5px",color:"#7A786F",fontWeight:400}}>{t.domains[k]}</div></div>))}
        </div>
        <div style={{display:"flex",gap:28,alignItems:"center",color:"#4A4840",fontSize:12,fontWeight:300,marginBottom:36}}>
          <span>⏱ {t.time}</span><span style={{width:1,height:14,background:"#1E1E24"}}/><span>{t.questions}</span><span style={{width:1,height:14,background:"#1E1E24"}}/><span>{t.timed}</span>
        </div>
        <button onClick={startTest} style={{padding:"18px 56px",fontSize:14,fontWeight:500,background:"linear-gradient(135deg,#D4A843,#C49530)",color:"#08080C",borderRadius:6,letterSpacing:"3px",textTransform:"uppercase",transition:"all .35s",boxShadow:"0 4px 24px rgba(212,168,67,.2)"}}
          onMouseEnter={e=>{e.target.style.transform="translateY(-2px)";e.target.style.boxShadow="0 8px 32px rgba(212,168,67,.3)";}}
          onMouseLeave={e=>{e.target.style.transform="translateY(0)";e.target.style.boxShadow="0 4px 24px rgba(212,168,67,.2)";}}>{t.startBtn}</button>
        <div style={{marginTop:36,display:"flex",alignItems:"center",gap:12}}>
          <div style={{display:"flex"}}>{[1,2,3,4,5].map(i=>(<div key={i} style={{width:28,height:28,borderRadius:"50%",background:`hsl(${i*50+20},30%,${25+i*5}%)`,border:"2px solid #08080C",marginLeft:i>1?-8:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:600,color:"#AAA"}}>{String.fromCharCode(64+i)}</div>))}</div>
          <span style={{fontSize:12,color:"#4A4840"}}>{t.socialProof}</span>
        </div>
        <p style={{position:"absolute",bottom:20,fontSize:10,color:"#1E1E24",textAlign:"center",maxWidth:400,lineHeight:1.6}}>{t.disclaimer}</p>
      </div>
    </div>
  );}
  ...
  if(phase==="test"&&q){return(
    <div style={bg}><style>{css}</style><div style={gr}/>
      <div style={{position:"fixed",top:0,left:0,right:0,height:3,background:"#12121A",zIndex:10}}><div style={{height:"100%",background:"linear-gradient(90deg,#45B7AA,#D4A843)",width:`${(currentQ/questions.length)*100}%`,transition:"width .5s"}}/></div>
      <div style={{...ct,justifyContent:"flex-start",paddingTop:48}}>
        <div style={{display:"flex",justifyContent:"space-between",width:"100%",maxWidth:600,marginBottom:12}}>
          <div style={{fontSize:11,color:D_COLORS[q.domain],letterSpacing:"3px",textTransform:"uppercase",display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14}}>{D_ICONS[q.domain]}</span>{t.domains[q.domain]}</div>
          <div style={{fontSize:13,color:"#4A4840",fontFamily:"'IBM Plex Mono',monospace",fontWeight:300}}>{currentQ+1}<span style={{opacity:.3}}> / </span>{questions.length}</div>
        </div>
        <div style={{width:"100%",maxWidth:600,height:2,background:"#12121A",borderRadius:1,marginBottom:12}}><div style={{height:"100%",background:tc,borderRadius:1,width:`${tp}%`,transition:"width 1s linear,background .5s"}}/></div>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:tc,marginBottom:40,fontWeight:300,opacity:.6}}>{timeLeft}s</div>
        <div style={{maxWidth:600,width:"100%",opacity:fadeIn?1:0,transform:fadeIn?"translateY(0)":"translateY(20px)",transition:"all .45s"}}>
          <div style={{background:"rgba(255,255,255,.015)",border:"1px solid rgba(255,255,255,.04)",borderRadius:14,padding:"36px 32px",marginBottom:20}}>
            <h2 style={{fontSize:"clamp(18px,3vw,22px)",fontWeight:400,lineHeight:1.6,color:"#E2E0DB",textAlign:"center"}}>{q.question}</h2>
          </div>
          <div style={{display:"grid",gap:10}}>
            {q.options.map((opt,idx)=>{
              let ob="rgba(255,255,255,.02)",oB="1px solid rgba(255,255,255,.05)",oC="#B8B6B0";
              if(showFB){if(idx===q.correct){ob="rgba(46,189,127,.12)";oB="1px solid rgba(46,189,127,.4)";oC="#2EBD7F";}else if(idx===selected){ob="rgba(212,93,93,.12)";oB="1px solid rgba(212,93,93,.4)";oC="#D45D5D";}}
              else if(selected===idx){ob="rgba(212,168,67,.08)";oB="1px solid rgba(212,168,67,.3)";}
              return(<button key={idx} onClick={()=>doSelect(idx)} style={{padding:"16px 22px",background:ob,border:oB,borderRadius:10,color:oC,fontSize:15,fontWeight:400,textAlign:"left",transition:"all .2s",display:"flex",alignItems:"center",gap:16,cursor:showFB?"default":"pointer"}}>
                <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#4A4840",minWidth:18}}>{String.fromCharCode(65+idx)}</span>{opt}
              </button>);
            })}
          </div>
          {showFB&&selected===null&&<div style={{textAlign:"center",marginTop:20,color:"#D45D5D",fontSize:13,animation:"fadeIn .3s",fontWeight:300}}>{t.timeUp}</div>}
        </div>
        <div style={{position:"fixed",bottom:20,left:"50%",transform:"translateX(-50%)",display:"flex",gap:5,alignItems:"center"}}>
          {[1,2,3,4,5].map(i=><div key={i} style={{width:5,height:5,borderRadius:"50%",background:i<=Math.ceil(q.difficulty*5)?"#D4A843":"#1E1E24",transition:"background .3s"}}/>)}
          <span style={{fontSize:10,color:"#1E1E24",marginLeft:6}}>{t.difficulty}</span>
        </div>
      </div>
    </div>
  );}
  ...
  if(phase==="calculating"){return(
    <div style={bg}><style>{css}</style><div style={gr}/>
      <div style={{...ct,animation:"fadeIn .5s"}}>
        <div style={{width:44,height:44,border:"2px solid #1E1E24",borderTopColor:"#D4A843",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
        <p style={{marginTop:28,color:"#4A4840",fontSize:12,letterSpacing:"4px",fontWeight:300}}>{t.analyzing}</p>
        <div style={{marginTop:12,display:"flex",gap:6}}>{[0,.3,.6].map((d,i)=><div key={i} style={{width:4,height:4,borderRadius:"50%",background:"#D4A843",animation:`pulse 1.2s ease-in-out ${d}s infinite`}}/>)}</div>
      </div>
    </div>
  );}

  // RESULTS
  if(phase==="results"&&iqResult){const pc=pctl(iqResult.estimate);return(
    <div style={bg}><style>{css}</style><div style={gr}/>
      <div style={{...ct,justifyContent:"flex-start",paddingTop:48}}>
        <div style={{fontSize:12,fontWeight:500,letterSpacing:"4px",color:"#D4A843",marginBottom:36}}>{t.brand}<span style={{color:"#45B7AA"}}>{t.brandAccent}</span> — {t.results}</div>
        <div style={{textAlign:"center",marginBottom:12,animation:"countUp .8s ease-out"}}>
          <div style={{fontSize:"clamp(72px,14vw,108px)",fontFamily:"'Playfair Display',serif",fontWeight:300,lineHeight:1,background:"linear-gradient(180deg,#D4A843 20%,#8B6914 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:"-4px"}}>{iqResult.estimate}</div>
          <div style={{fontSize:13,color:"#7A786F",fontWeight:300,marginTop:6,letterSpacing:"1px"}}>{t.iqEstimated}</div>
        </div>
        <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.05)",borderRadius:10,padding:"18px 36px",marginBottom:32,textAlign:"center",animation:"fadeUp .6s ease-out .3s both"}}>
          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:22,fontWeight:400,color:"#E2E0DB",letterSpacing:"2px"}}>{iqResult.lower} — {iqResult.upper}</div>
          <div style={{fontSize:11,color:"#4A4840",marginTop:6,letterSpacing:"2px"}}>{t.confidenceInterval}</div>
        </div>
        {/* Bell curve */}
        <div style={{maxWidth:520,width:"100%",marginBottom:32,animation:"fadeUp .6s ease-out .5s both"}}>
          <svg viewBox="0 0 520 110" style={{width:"100%",height:"auto"}}>
            <defs><linearGradient id="cg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#45B7AA" stopOpacity=".15"/><stop offset="100%" stopColor="#45B7AA" stopOpacity="0"/></linearGradient><linearGradient id="hg" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D4A843" stopOpacity=".25"/><stop offset="100%" stopColor="#D4A843" stopOpacity="0"/></linearGradient></defs>
            <path d={(()=>{let p="M 0 100";for(let x=0;x<=520;x+=2){const iq=40+(x/520)*120,z=(iq-100)/15;p+=` L ${x} ${100-Math.exp(-.5*z*z)*88}`;}return p+" L 520 100 Z";})()}fill="url(#cg)"stroke="rgba(69,183,170,.25)"strokeWidth="1"/>
            <path d={(()=>{const ix=iq=>Math.max(0,Math.min(520,((iq-40)/120)*520));let p=`M ${ix(iqResult.lower)} 100`;for(let x=ix(iqResult.lower);x<=ix(iqResult.upper);x+=2){const iq=40+(x/520)*120,z=(iq-100)/15;p+=` L ${x} ${100-Math.exp(-.5*z*z)*88}`;}return p+` L ${ix(iqResult.upper)} 100 Z`;})()}fill="url(#hg)"stroke="rgba(212,168,67,.5)"strokeWidth="1"/>
            {(()=>{const ex=((iqResult.estimate-40)/120)*520,z=(iqResult.estimate-100)/15;return<line x1={ex}y1={100-Math.exp(-.5*z*z)*88-4}x2={ex}y2={100}stroke="#D4A843"strokeWidth="1.5"strokeDasharray="3 3"/>;})()}
            {[55,70,85,100,115,130,145].map(iq=><text key={iq}x={((iq-40)/120)*520}y={109}fill={iq===100?"#5A5850":"#2A2A30"}fontSize="8"textAnchor="middle"fontFamily="IBM Plex Mono">{iq}</text>)}
          </svg>
        </div>
        {/* Basic stats */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,maxWidth:460,width:"100%",marginBottom:36,animation:"fadeUp .6s ease-out .6s both"}}>
          {[{l:t.classification,v:iqLabel(iqResult.estimate,t),m:false},{l:t.percentile,v:`${pc}%`,m:true},{l:t.correct,v:`${iqResult.totalCorrect}/${iqResult.totalQuestions}`,m:true}].map((s,i)=>(
            <div key={i}style={{textAlign:"center",padding:"16px 12px",background:"rgba(255,255,255,.02)",borderRadius:10,border:"1px solid rgba(255,255,255,.04)"}}>
              <div style={{fontSize:s.m?18:14,fontWeight:500,color:"#E2E0DB",marginBottom:4,fontFamily:s.m?"'IBM Plex Mono',monospace":"inherit"}}>{s.v}</div>
              <div style={{fontSize:10,color:"#4A4840",textTransform:"uppercase",letterSpacing:"1.5px"}}>{s.l}</div>
            </div>
          ))}
        </div>
        ...
        {pw?(
          <div style={{maxWidth:480,width:"100%",animation:"fadeUp .6s ease-out .8s both"}}>
            <div style={{position:"relative",borderRadius:14,overflow:"hidden",marginBottom:24}}>
              <div style={{filter:"blur(8px)",opacity:.4,pointerEvents:"none",padding:24,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)"}}>
                <div style={{fontSize:11,color:"#4A4840",marginBottom:16,letterSpacing:"3px"}}>{t.domainPerf.toUpperCase()}</div>
                {Object.keys(D_COLORS).map(k=>(<div key={k}style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}><div style={{width:100,fontSize:11,color:"#666"}}>{t.domains[k]}</div><div style={{flex:1,height:6,background:"#1A1A1F",borderRadius:3}}><div style={{width:`${40+Math.random()*50}%`,height:"100%",background:D_COLORS[k],borderRadius:3}}/></div></div>))}
                <div style={{marginTop:20,fontSize:13,color:"#666",lineHeight:1.7}}>Lorem ipsum dolor sit amet consectetur...</div>
              </div>
              <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:"rgba(8,8,12,.7)",backdropFilter:"blur(2px)"}}>
                ...
                <div style={{marginTop:16,textAlign:"center"}}>
                  ...
                  {!emailOk?(<div style={{display:"flex",gap:8,justifyContent:"center"}}>
                    <input type="email"placeholder={t.emailPlaceholder}value={email}onChange={e=>setEmail(e.target.value)}onKeyDown={e=>{if(e.key==="Enter"&&email.includes("@"))setEmailOk(true);}}style={{padding:"8px 14px",fontSize:13,background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:5,color:"#E2E0DB",width:200}}/>
                    <button onClick={()=>{if(email.includes("@"))setEmailOk(true);}}style={{padding:"8px 16px",fontSize:12,background:"rgba(69,183,170,.15)",border:"1px solid rgba(69,183,170,.3)",borderRadius:5,color:"#45B7AA",fontWeight:500}}>{t.send}</button>
                  </div>):<div style={{fontSize:12,color:"#45B7AA"}}>{t.emailSent}</div>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div style={{maxWidth:480,width:"100%",animation:"fadeUp .6s ease-out"}}>
            <div style={{padding:24,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)",borderRadius:12,marginBottom:20}}>
              <div style={{fontSize:11,letterSpacing:"3px",textTransform:"uppercase",color:"#4A4840",marginBottom:18}}>{t.domainPerf}</div>
              {Object.entries(iqResult.domainScores).map(([k,s])=>(<div key={k}style={{display:"flex",alignItems:"center",gap:12,marginBottom:10}}>
                <div style={{width:110,fontSize:11,color:"#7A786F",textAlign:"right"}}>{t.domains[k]}</div>
                <div style={{flex:1,height:6,background:"#12121A",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${s}%`,background:D_COLORS[k],borderRadius:3,animation:"slideWidth 1s ease-out"}}/></div>
                <div style={{width:36,fontSize:12,color:"#4A4840",fontFamily:"'IBM Plex Mono',monospace"}}>{s}%</div>
              </div>))}
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
              <div style={{padding:18,background:"rgba(46,189,127,.04)",border:"1px solid rgba(46,189,127,.1)",borderRadius:10}}>
                <div style={{fontSize:10,color:"#2EBD7F",letterSpacing:"2px",marginBottom:10}}>{t.strengths}</div>
                {iqResult.strengths.map(s=><div key={s}style={{fontSize:12,color:"#B8B6B0",marginBottom:4}}>{D_ICONS[s]} {t.domains[s]}</div>)}
              </div>
              <div style={{padding:18,background:"rgba(212,93,93,.04)",border:"1px solid rgba(212,93,93,.1)",borderRadius:10}}>
                <div style={{fontSize:10,color:"#D45D5D",letterSpacing:"2px",marginBottom:10}}>{t.areasImprove}</div>
                {iqResult.weaknesses.map(w=><div key={w}style={{fontSize:12,color:"#B8B6B0",marginBottom:4}}>{D_ICONS[w]} {t.domains[w]}</div>)}
              </div>
            </div>
            <div style={{padding:22,background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.04)",borderRadius:12,marginBottom:20}}>
              <div style={{fontSize:10,letterSpacing:"3px",color:"#4A4840",marginBottom:10}}>{t.interpretation}</div>
              <p style={{fontSize:13.5,color:"#7A786F",lineHeight:1.75,fontWeight:300}}>{t.interpretText(iqResult.lower,iqResult.upper,pc,iqResult.avgTime<18,t.domains[iqResult.strengths[0]]?.toLowerCase(),t.domains[iqResult.weaknesses[0]]?.toLowerCase())}</p>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
              {[{l:t.avgTime,v:`${iqResult.avgTime}s`},{l:t.rawScore,v:`${iqResult.rawScore}%`}].map((s,i)=>(<div key={i}style={{padding:14,textAlign:"center",background:"rgba(255,255,255,.02)",borderRadius:8,border:"1px solid rgba(255,255,255,.03)"}}><div style={{fontSize:18,fontFamily:"'IBM Plex Mono',monospace",fontWeight:400,color:"#E2E0DB"}}>{s.v}</div><div style={{fontSize:10,color:"#4A4840",letterSpacing:"1px",marginTop:2}}>{s.l.toUpperCase()}</div></div>))}
            </div>
          </div>
        )}

        <div style={{display:"flex",gap:12,marginTop:16,flexWrap:"wrap",justifyContent:"center",animation:"fadeUp .6s ease-out 1s both"}}>
          <button onClick={()=>{const txt=t.shareText(iqResult.estimate,iqResult.lower,iqResult.upper);navigator.share?navigator.share({title:"NexusIQ",text:txt}):navigator.clipboard?.writeText(txt);}}style={{padding:"12px 24px",fontSize:12,fontWeight:500,background:"rgba(69,183,170,.1)",color:"#45B7AA",border:"1px solid rgba(69,183,170,.2)",borderRadius:6,letterSpacing:"1.5px"}}>{t.share}</button>
          <button onClick={startTest}style={{padding:"12px 24px",fontSize:12,fontWeight:500,background:"transparent",color:"#4A4840",border:"1px solid rgba(255,255,255,.06)",borderRadius:6,letterSpacing:"1.5px"}}>{t.retry}</button>
        </div>
        <p style={{marginTop:48,fontSize:10,color:"#1E1E24",textAlign:"center",maxWidth:380,lineHeight:1.6}}>{t.footerDisclaimer}</p>
      </div>
    </div>
  );}
